function selectindex(components, point) {
  for (let i = 0; i < components.length; i++) {
    if (collidePointPoly(point.x, point.y, components[i].vertices)) {
      ...
      components[i].type == "CLens" ||
      components[i].type == "DLens" ||
      components[i].type == "DGrating" ||
      components[i].type == "CGrating" ||
      components[i].type == "Prism" ||
      components[i].type == "Sun"
    ) {
...
function findEditableData(obj) {
  if (obj.type == "reflector") {
    return [
      obj,
      ["Position", obj.pos],
      ["Angle", obj.coords.theta],
      ["Width", obj.coords.magnitude * 2],
    ...
    return [
      ...
      ["Refractive Coefficients", obj.rcoeffs],
    ];
    ...
    return [
      ...
      ["Angle", obj.coords.theta],
      ["Width", obj.coords.magnitude * 2],
      ["Maximum Order", obj.maxorder],
      ["Slit Separation Distance", obj.slitdist],
    ];
    ...
    return [
      ...
      ["Angle", obj.coords.theta],
      ["Width", obj.coords.magnitude * 2],
      ["Number of different wavelengths", obj.number],
    ];
